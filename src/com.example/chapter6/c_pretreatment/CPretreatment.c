...
///下一行告诉 CPP 从本地目录中获取 stdint.h，并添加内容到当前的源文件中。
...
#define token_paster(n) printf("token" #n "= %d ",token##n);
...
//}
//我们可以使用宏重写上面的代码，如下：
///在使用带有参数的宏之前，必须使用 #define 指令定义。参数列表是括在圆括号内，且必须紧跟在宏名称的后边。宏名称和右圆括号之间不允许有空格。例如：
#define square(x) ((x) * (x))
