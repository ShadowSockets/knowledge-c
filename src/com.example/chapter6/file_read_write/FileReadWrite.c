...
//
//一个文件，无论它是文本文件还是二进制文件，都是代表了一系列的字节。C 语言不仅提供了访问顶层的函数，
...
int main() {
    ...
    fp = fopen("D:\\1.txt", "w+");
    fprintf(fp, "this is testing for c \n");
    fputs("line two \n", fp);
    fclose(fp);
    //当上面的代码被编译和执行时，它会在 D: 目录中创建一个新的文件 test.txt，并使用两个不同的函数写入两行。接下来让我们来读取这个文件
